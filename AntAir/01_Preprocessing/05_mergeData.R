#This script merges predictor variables with air temeprature emasurements
rm(list=ls())
library(raster)
library(lubridate)
#mainpath <- "/media/memory02/users/hmeyer/Antarctica/ReModel2017/" #183
...
extracted <- list.files(paste0(rdatapath,"extracted/"),
                        full.names = TRUE,pattern="extract")
extracted_VIS <- list.files(paste0(rdatapath,"extracted_VIS/"),
                            full.names = TRUE,pattern="extract")
...
for (i in 1:length(extracted)){
  ...
  extr_agg <- aggregate(extr$LST,FUN=mean,na.rm=T,
                        by=list(extr$Date,extr$Station,extr$daynight))
...
results$Date <- as.Date(as.numeric(substr(results$Date,5,7)), 
                         origin=as.Date(paste0(as.numeric(
                           substr(results$Date,1,4))-1,"-12-31")))
results_merged <- merge(results,dailydat,by.x=c("Date","Station"),
                        by.y=c("Date","Name"))
...
results$Date <- as.Date(as.numeric(substr(results$Date,5,7)), 
                        origin=as.Date(paste0(as.numeric(
                          substr(results$Date,1,4))-1,"-12-31"))) 
results_merged <- merge(results_merged,results,by.x=c("Date","Station"),
                        by.y=c("Date","Station"))
...
results_merged <- merge(results_merged,aux,by.x=c("DOY","Station"),
                        by.y=c("Doy","Station"))
